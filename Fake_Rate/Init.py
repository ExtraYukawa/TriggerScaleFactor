...
def FakeRateFilesIn(year:str):
    ...
    if year == '2017':
        ...
        postfixs=['B','C','D','E','F']  
        FilesIn['Data']['Electron'] = [os.path.join(path,'SingleEG'+postfix+'.root') for postfix in postfixs]
        FilesIn['Data']['Muon'] = [os.path.join(path,'DoubleMuon'+postfix+'.root') for postfix in postfixs]
        FilesIn['MC']['Lepton']['DY']['path'] = os.path.join(path,'DY.root')
    elif year =='2018':
        path='/eos/user/m/melu/TTC_fakerate_v9_2018'
        postfixs=['A','B','C','D'] 
        FilesIn['Data']['Electron'] = [os.path.join(path,'EGamma'+postfix+'.root') for postfix in postfixs]
        FilesIn['Data']['Muon'] = [os.path.join(path,'DoubleMu'+postfix+'.root') for postfix in postfixs]
        FilesIn['MC']['Lepton']['DY']['path'] = os.path.join(path,'DYnlo.root')
