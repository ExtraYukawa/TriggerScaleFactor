...
class DrellYanRDataFrame():
    ...
    def Run(self):
        ...
        if self.__year == '2018' and self.__veto:
            ...
            veto_condition='veto_hemregion(run,Jet_phi,Jet_eta)'
        ...
        for p in self.__Process:
            for MC in self.__FilesIn['MC'][p].keys():
                Filtering(self.__dfs['MC'][MC],HistSettings,veto_condition)
        
        for Sample_Name in self.__HLT_Path[self.__channel].keys():
            Filtering(self.__dfs['Data'][Sample_Name] , HistSettings,veto_condition)
